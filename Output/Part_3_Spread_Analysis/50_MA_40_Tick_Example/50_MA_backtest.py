# deep_dive_mean_reversion_50ma_with_dates.py

# This script performs a focused backtest on the 50-period MA mean reversion strategy.
# It now includes custom date labels on the x-axis of the main trades chart
# to provide time context while keeping the plot free of visual gaps.
...
def run_mean_reversion_backtest(df, ma_period, deviation_threshold):
    """Simulates the mean reversion strategy."""
...
def plot_trades_chart_with_dates(strategy_df, trades_df, num_labels=10):
    """
    Creates and saves a chart showing all trade entries and exits,
    with custom date labels on the x-axis.
    """
    if trades_df.empty:
    ...
    # Plot the main lines against the clean (0, 1, 2...) index
    ...
    # Map entry/exit times to the clean index positions for plotting
    ...
    # 1. Get evenly spaced positions along the index.
    tick_positions = np.linspace(0, len(strategy_df) - 1, num_labels, dtype=int)
    
    # 2. Look up the actual timestamps at these specific positions in the strategy DataFrame.
    # Note: We use .iloc to access by integer position, which is safe because the index is now 0-based.
    tick_labels_ts = strategy_df['timestamp'].iloc[tick_positions]
    
    # 3. Format these timestamps into readable date strings.
...
def process_and_display_equity_curve(trades_df):
    """Creates equity curve plot and prints the data table."""
    if trades_df.empty:
