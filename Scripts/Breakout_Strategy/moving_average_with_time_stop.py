# 09_breakout_with_time_stop.py

# This script tests the full breakout strategy with a time-based stop loss.
#
# Strategy Logic:
# - Entry (Long):   Spread is 40 points ABOVE the Moving Average.
# - Entry (Short):  Spread is 40 points BELOW the Moving Average.
# - Exit Condition 1 (Profit Target): Spread crosses back over the Moving Average.
# - Exit Condition 2 (Time Stop): The specified time limit is reached.
# The trade is closed if EITHER exit condition is met first.
...
def run_breakout_with_time_stop(df, ma_period, deviation_threshold, time_limit_periods, time_limit_label):
    """
    This is the main analysis function. It runs the breakout backtest for a specific
    combination of MA period and time limit.
    """
    # --- Part 1: Setup the Strategy Indicators ---
    print(f"\n{'='*50}")
    print(f"--- Testing Breakout MA={ma_period} | Time Limit={time_limit_label} ---")
    print(f"{'='*50}")
    ...
    for i, row in strategy_df.iterrows():
        ...
        if in_trade:
            # A.1: Check for the MA Cross exit condition.
            is_ma_cross_exit = False
            # For a Long trade, the exit is when price falls back below the MA.
            if trade_type == 'Long' and row['Spread'] < row['Spread_MA']:
                is_ma_cross_exit = True
            # For a Short trade, the exit is when price rallies back above the MA.
            elif trade_type == 'Short' and row['Spread'] > row['Spread_MA']:
                is_ma_cross_exit = True

            # A.2: Check for the Time-Out exit condition.
            ...
            if is_ma_cross_exit or is_time_out_exit:
                ...
                # Determine the reason for the exit for our analysis.
        ...
        if not in_trade:
            # Check for a Long breakout entry.
            if row['Spread'] > row['Upper_Band']:
                in_trade = True
                trade_type = 'Long'
                entry_price = row['Spread']
                entry_time = row['timestamp']
                entry_index = i # Record the entry index for the time stop.
                
            # Check for a Short breakdown entry.
    ...
    win_rate = (trades_df[trades_df['profit'] > 0]['profit'].count() / number_of_trades) * 100
    ...
    # Filter exits by reason for distinct plotting.
    ...
    plt.title(f"Breakout Strategy: MA={ma_period}, Time Limit={time_limit_label}", fontsize=16)
